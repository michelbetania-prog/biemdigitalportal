...
Deno.serve(async request=>{
  ...
  try{
    ...
    for(const recipient of clientRecipients){
      ...
      const subject=subjects[notification_type], notification={client_id:clientId,user_id:recipient.id,notification_type,recipient_email:recipient.email,subject,related_entity_type:notification_type.split('_')[0],related_entity_id,status:'pending'}
      ...
      try{
        ...
        const html=`<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#243128"><p>Hola ${esc(recipient.full_name||client?.brand_name||'')}</p><h1>${esc(subject)}</h1><h2>${esc(title)}</h2><p style="line-height:1.6">${esc(detail)}</p><a href="${site}${action}" style="display:inline-block;background:#31563e;color:white;padding:12px 18px;border-radius:10px;text-decoration:none">Ver en el portal</a><p style="margin-top:28px;color:#718078">Biem Digital · Portal privado</p></div>`
    ...
    if(notification_type==='request_created'&&actor.role==='client'){
      ...
      for(const member of (staff||[]).filter(item=>item.email&&(item.role==='admin'||assigned.has(item.id)))){
        ...
        try{if(!resendKey)throw new Error('RESEND_API_KEY no está configurada.');const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${resendKey}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[member.email],subject,html:`<h1>${esc(subject)}</h1><h2>${esc(title)}</h2><p>${esc(detail)}</p><a href="${site}/admin/dashboard">Ver solicitud en admin</a>`})});const payload=await response.json();if(!response.ok)throw new Error(payload.message||'Error de Resend');await db.from('email_notifications').update({status:'sent',sent_at:new Date().toISOString(),provider_message_id:payload.id}).eq('id',log.id)}catch(error){await db.from('email_notifications').update({status:'failed',error_message:error.message}).eq('id',log.id)}}
    }
