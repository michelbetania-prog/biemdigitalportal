...
function Dashboard({ workspace, setActive, profile }) {
  ...
  const pendingInvoices=data.invoices.filter(item=>['pending','overdue'].includes(item.status)).length
  ...
  const cards=[
    ['Clientes activos',activeClients,'Cuentas en servicio',Users,'green','clients'],
    ['Entregables pendientes',pending,'Producción abierta',Clock3,'amber','deliverables'],
    ['Solicitudes recientes',newRequests,'Nuevas peticiones',MessageCircle,'lilac','requests'],
    ['Facturas pendientes',pendingInvoices,'Cobros por resolver',Receipt,'coral','billing'],
  ]
  const activity=[
    ...data.requests.slice(0,4).map(item=>({id:`r-${item.id}`,title:item.clients?.brand_name||'Cliente',text:`solicitó ${item.request_type}`,time:formatDate(item.created_at),icon:MessageCircle,target:'requests'})),
    ...data.deliverables.slice(0,4).map(item=>({id:`d-${item.id}`,title:item.clients?.brand_name||'Cliente',text:`actualizó ${item.name}`,time:formatDate(item.updated_at),icon:Image,target:'deliverables'})),
    ...data.invoices.slice(0,3).map(item=>({id:`i-${item.id}`,title:item.clients?.brand_name||'Cliente',text:`factura ${labels[item.status]}`,time:formatDate(item.updated_at),icon:Receipt,target:'billing'})),
  ].slice(0,7)
  ...
  return <div className="admin-page dashboard-page"><section className="dashboard-hero admin-panel"><div><span className="admin-eyebrow">RESUMEN GENERAL</span><h1>Hola, {profile.full_name?.split(' ')[0]||'equipo'}</h1><p>Panel operativo de BIEM con datos sincronizados desde Supabase.</p></div><div className="dashboard-quick-actions">{quickActions.map(([label,target,Icon])=><button key={label} onClick={()=>setActive(target)}><Icon size={16}/>{label}</button>)}</div></section><Feedback error={workspace.error} notice={workspace.notice} loading={workspace.loading}/><section className="admin-kpi-grid compact">{cards.map(([label,value,note,Icon,tone,target])=><button className="admin-kpi-card" key={label} onClick={()=>setActive(target)}><div className={`admin-kpi-icon ${tone}`}><Icon size={18}/></div><span>{label}</span><strong>{value}</strong><p>{note}</p></button>)}</section><section className="admin-dashboard-grid modular"><div className="admin-panel"><div className="admin-panel-head"><div><span className="admin-eyebrow">CLIENTES</span><h2>Clientes activos</h2></div><button onClick={()=>setActive('clients')}>Ver módulo</button></div><div className="visual-list">{data.clients.filter(item=>item.status==='active').slice(0,5).map(item=><button key={item.id} onClick={()=>setActive('clients')}><div className="brand-avatar">{initials(item.brand_name)}</div><span><strong>{item.brand_name}</strong><small>{item.name||'Contacto pendiente'}</small></span><Badge value={item.status}/></button>)}{!activeClients&&<GuideExample guideKey="admin.clients" canDismiss={workspace.canWrite}/>}</div></div><div className="admin-panel"><div className="admin-panel-head"><div><span className="admin-eyebrow">PRODUCCIÓN</span><h2>Entregables pendientes</h2></div><button onClick={()=>setActive('deliverables')}>Ver todos</button></div><div className="visual-list">{data.deliverables.filter(item=>['pending','in_progress','internal_review','client_review'].includes(item.status)).slice(0,5).map(item=><button key={item.id} onClick={()=>setActive('deliverables')}><div className="activity-icon"><Grid2X2 size={15}/></div><span><strong>{item.name}</strong><small>{item.clients?.brand_name||'Cliente'} · {formatDate(item.due_date)}</small></span><Badge value={item.status}/></button>)}</div></div><div className="admin-panel"><div className="admin-panel-head"><div><span className="admin-eyebrow">SOLICITUDES</span><h2>Solicitudes recientes</h2></div><button onClick={()=>setActive('requests')}>Responder</button></div><div className="visual-list">{data.requests.slice(0,5).map(item=><button key={item.id} onClick={()=>setActive('requests')}><div className="activity-icon"><MessageCircle size={15}/></div><span><strong>{item.request_type}</strong><small>{item.clients?.brand_name||'Cliente'} · {formatDate(item.created_at)}</small></span><Badge value={item.status}/></button>)}</div></div><div className="admin-panel"><div className="admin-panel-head"><div><span className="admin-eyebrow">AGENDA</span><h2>Próximas reuniones</h2></div><button onClick={()=>setActive('meetings')}>Agendar</button></div><div className="visual-list">{nextMeetings.map(item=><button key={item.id} onClick={()=>setActive('meetings')}><div className="attention-date">{new Date(item.start_time).getDate()||'—'}<span>{formatDate(item.start_time).slice(3,6)}</span></div><span><strong>{item.title}</strong><small>{item.client?.brand_name||'Cliente'} · {item.google_meet_link?'Meet listo':'Sin enlace'}</small></span><Badge value={item.status}/></button>)}</div></div><div className="admin-panel"><div className="admin-panel-head"><div><span className="admin-eyebrow">COBROS</span><h2>Facturas pendientes</h2></div><button onClick={()=>setActive('billing')}>Gestionar</button></div><div className="visual-list">{data.invoices.filter(item=>['pending','overdue'].includes(item.status)).slice(0,5).map(item=><button key={item.id} onClick={()=>setActive('billing')}><div className="activity-icon"><DollarSign size={15}/></div><span><strong>{item.invoice_number}</strong><small>{item.clients?.brand_name||'Cliente'} · {formatMoney(item.amount,item.currency)}</small></span><Badge value={item.status}/></button>)}</div></div><div className="admin-panel activity-panel"><div className="admin-panel-head"><div><span className="admin-eyebrow">ACTIVIDAD</span><h2>Actividad reciente</h2></div></div><div className="activity-list">{activity.map(item=>{const Icon=item.icon;return <button className="activity-item" key={item.id} onClick={()=>setActive(item.target)}><div className="activity-icon"><Icon size={15}/></div><div><p><strong>{item.title}</strong> {item.text}</p><span>{item.time}</span></div><ArrowRight size={14}/></button>})}{!activity.length&&<GuideExample guideKey="admin.dashboard" canDismiss={workspace.canWrite}/>}</div></div></section></div>
}
